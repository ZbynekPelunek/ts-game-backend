...
import { Request_Inventory_GET_all_query, Response_Inventory_GET_all, Request_Inventory_GET_item_param, Response_Inventory_GET_one, Request_Inventory_POST_body, Response_Inventory_POST, Request_Inventory_PATCH_param, Request_Inventory_PATCH_body, Response_Inventory_PATCH, Response_Item_GET_one, InventoryFrontend, InventoryActions } from '../../../shared/src';

export const inventoryRouter = express.Router();

inventoryRouter.get('', async (req: Request<{}, {}, {}, Request_Inventory_GET_all_query>, res: Response<Response_Inventory_GET_all>) => {
...
inventoryRouter.get('/:inventoryId', async (req: Request<Request_Inventory_GET_item_param>, res: Response<Response_Inventory_GET_one>) => {
  ...
  return res.status(200).json({ success: true, inventory: transformResponse(inventory) });
})

inventoryRouter.post('', async (req: Request<{}, {}, Request_Inventory_POST_body, { action: string }>, res: Response<Response_Inventory_POST>) => {
  try {
    ...
    if (action === InventoryActions.NEW) {
      ...
      const inventoryDbResponse = await InventoryModel.create(defaultCharacterInventory);
      response = inventoryDbResponse.map(ii => {
        return transformResponse(ii);
      });
    } else {
      const { characterId, slot, itemId } = req.body;
      const inventoryDbResponse = await InventoryModel.create({ characterId, slot, itemId });
...
inventoryRouter.patch('/:inventoryId', async (req: Request<Request_Inventory_PATCH_param, {}, Request_Inventory_PATCH_body>, res: Response<Response_Inventory_PATCH>) => {
  const { inventoryId } = req.params;
  const { characterId, itemId, amount, previousItemSlot } = req.body;

  try {
    console.log('Looking for item data...');
    const item = await axios.get<Response_Item_GET_one>(`http://localhost:3000/api/v1/items/${itemId}`);
    ...
    if (!inventorySlotItem) {
      return res.status(404).json({ success: false, error: `Inventory item with id '${inventoryId}' does not exist` })
    }
    ...
    if (inventorySlotItem.itemId === item.data.item.itemId) {
      ...
      updateRes = await InventoryModel.findByIdAndUpdate(inventoryId, { $inc: { amount } }, { returnDocument: 'after' });
    ...
    if (!inventorySlotItem.itemId) {
      ...
      updateRes = await InventoryModel.findByIdAndUpdate(inventoryId, { $set: { amount, characterId, itemId } }, { returnDocument: 'after' });
      console.log('Update db response: ', updateRes);
      ...
      if (previousItemSlot) {
        ...
        updateRes = await InventoryModel.findByIdAndUpdate(inventoryId, { itemId, amount }, { returnDocument: 'after' });
        console.log('Update db response: ', updateRes);

        await InventoryModel.findOneAndUpdate({ slot: previousItemSlot }, { itemId: inventorySlotItem.itemId, amount: inventorySlotItem.amount });
        ...
        const allCharacterItemSlots = await InventoryModel.find({ characterId });
        ...
        updateRes = await InventoryModel.findOneAndUpdate({ slot: firstFreeSlot.slot, characterId }, { itemId, amount }, { returnDocument: 'after' });
        console.log('Update db response: ', updateRes);
...
const transformResponse = (databaseResponse: InventorySchema & Document): InventoryFrontend => {
  return {
